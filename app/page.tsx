...
const getStageColor = (stage: string) => {
  switch (stage) {
    case '幸存者': return 'text-stone-500 bg-stone-100 border-stone-200';
    case '狩猎者': return 'text-emerald-700 bg-emerald-50 border-emerald-200';
    case '开拓者': return 'text-blue-700 bg-blue-50 border-blue-200';
    case '领主': return 'text-purple-800 bg-purple-50 border-purple-200';
    case '征服者': return 'text-orange-800 bg-orange-50 border-orange-200';
      ...
      <div className="flex justify-between items-start mb-3">
        ...
        <div className="flex flex-col items-end gap-1">
           <div className="text-sm font-bold text-amber-900 flex items-center gap-1">
             <div className="w-4 h-4 rounded-full border border-amber-900 flex items-center justify-center text-[10px]">资</div>
             {hero.gold}
           </div>
           <div className="flex items-center gap-2">
             ...
             <div className="flex flex-col items-end">
               <div className="flex items-center gap-1 text-amber-900 font-bold text-[10px]"><Brain size={10}/> {Math.floor(hero.godPower)}</div>
        ...
        <div className="bg-gradient-to-r from-amber-50 via-[#fcf9f2] to-white border-l-4 border-amber-700 p-3 shadow-sm rounded-r mb-3 animate-in slide-in-from-top duration-700">
           <div className="flex justify-between items-center mb-1">
              <span className="flex items-center gap-1.5 text-xs font-bold text-amber-900 tracking-wide font-serif">
                ...
                生存目标：{hero.currentQuest?.name.replace('【主线】', '')}
              </span>
        ...
        <div className="bg-white border border-stone-200 rounded p-2 shadow-sm flex flex-col gap-1 mb-2">
           <div className="flex justify-between text-[10px] text-stone-500 mb-1">
              <span className="flex items-center gap-1 font-bold text-stone-700 truncate max-w-[200px]">
                ...
                {hero.currentQuest ? hero.currentQuest.name : "自由探索"}
              </span>
  ...
  return (
    <div className="flex flex-col h-full relative">
      <div className="flex-1 overflow-y-auto p-5 space-y-4 scroll-smooth pb-20">
        ...
        {hero.logs.length === 0 && <div className="text-center text-stone-300 py-10 font-serif italic">日记本是空的...</div>}
      </div>
      <div className="p-4 bg-gradient-to-t from-[#fcf9f2] via-[#fcf9f2] to-transparent">
         <div className="flex justify-between gap-4">
          <button onClick={() => godAction('punish')} disabled={hero.godPower < 25} className="flex-1 h-12 border border-stone-200 rounded flex flex-col items-center justify-center gap-0 text-stone-600 hover:bg-stone-100 active:scale-95 bg-white shadow-sm disabled:opacity-50 disabled:cursor-not-allowed group">
            <span className="flex items-center gap-1 text-sm font-bold group-hover:text-red-900"><Zap size={14} /> 磨难</span>
          ...
          <button onClick={() => godAction('bless')} disabled={hero.godPower < 25} className="flex-1 h-12 border border-stone-200 rounded flex flex-col items-center justify-center gap-0 text-stone-600 hover:bg-stone-100 active:scale-95 bg-white shadow-sm disabled:opacity-50 disabled:cursor-not-allowed group">
            <span className="flex items-center gap-1 text-sm font-bold group-hover:text-amber-800"><Cloud size={14} /> 幸运</span>
            <span className="text-[10px] text-stone-400">消耗 25%</span>
          </button>
        </div>
      </div>
...
// ... (EquipSlot, AttributeRow, HeroView, BagView, MessagesView, TavernView, Home 保持不变，请确保代码完整性) ...
// (为节省篇幅，这里假设其他组件未变动，但你需要确保文件完整)
// ⚠️ 重要：请确保将之前的 EquipSlot, AttributeRow 等组件也复制过来。
    ...
    <div className="p-6 overflow-y-auto h-full space-y-6">
      <div className="bg-white p-4 rounded-lg shadow-sm border border-stone-100">
        ...
        <div className="flex items-center justify-between mb-4">
           ...
           <div className="text-center flex-1"><div className="text-xs text-stone-400">生存日</div><div className="font-bold text-stone-700">{hero.age}</div></div>
        </div>
        ...
        <div className="mb-4 bg-stone-50 p-3 rounded border border-stone-100 relative">
           ...
           <div className="text-xs font-bold text-stone-700 mb-2 flex items-center gap-1"><Info size={12}/> 状态</div>
           <div className="text-[11px] text-stone-600 leading-relaxed font-serif italic">
             “{hero.description || "无名幸存者。"}”
           </div>
        </div>

        <div className="mb-4 bg-stone-50 p-3 rounded border border-stone-100 relative">
           <div className="absolute top-2 right-2 text-stone-300 opacity-20"><Shield size={48}/></div>
           <div className="text-xs font-bold text-stone-700 mb-2 flex items-center gap-1"><Shirt size={12}/> 装备</div>
           <div className="text-[11px] text-stone-600 leading-relaxed font-serif italic mb-3">
             “{hero.equipmentDescription || "衣衫褴褛。"}”
           </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-stone-100 relative overflow-hidden">
        <h3 className="font-bold text-stone-800 mb-4 flex items-center gap-2"><UserPlus size={16}/> 同伴</h3>
        {hero.companion ? (
           <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                 ...
                 <div className="mt-2 text-[10px] text-amber-700 flex items-center gap-1"><Info size={10}/> 停留: {Math.max(0, Math.floor((hero.companionExpiry - Date.now()) / (1000 * 60 * 60)))} 小时</div>
              </div>
           </div>
        ) : (
           <div className="text-center py-6 text-stone-400 text-xs">独自一人。</div>
        )}
      </div>
      <div className="bg-white p-4 rounded-lg shadow-sm border border-stone-100">
         <h3 className="font-bold text-stone-800 mb-4 flex items-center gap-2"><BookOpen size={16}/> 技能</h3>
  ...
  return (
    <div className="p-4 h-full overflow-y-auto">
      <div className="flex justify-between items-center mb-4 px-2">
        <h3 className="font-bold text-stone-800">背包 ({hero.inventory.length}/20)</h3>
        <span className="text-[10px] text-stone-400 italic">自动消耗食物/药物</span>
      </div>
      <div className="space-y-2">
        {hero.inventory.map((item,i)=> (
          <div key={i} className="bg-white border border-stone-100 p-3 rounded flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] bg-stone-50 px-1 rounded text-stone-400">值{item.price}</span>
            </div>
          </div>
        ))}
      </div>
...
const MessagesView = memo(({ hero }: { hero: HeroState }) => { 
    ...
    return (<div className="p-4 h-full overflow-y-auto space-y-6"><div><h3 className="font-bold text-stone-800 mb-3 flex items-center gap-2 px-1"><MessageSquare size={16}/> 无线电/信号</h3>{rumors.length === 0 ? <div className="text-center text-stone-300 text-xs italic">暂无信号</div> : <div className="space-y-3">{rumors.map((msg)=><div key={msg.id} className="bg-amber-50 border border-amber-100 p-3 rounded-lg shadow-sm"><div className="flex justify-between items-start mb-1"><div className="font-bold text-amber-900 text-sm">{msg.title}</div><div className="text-[10px] text-amber-400">{msg.time}</div></div><div className="text-xs text-amber-800 leading-relaxed text-justify">{msg.content}</div></div>)}</div>}</div><div><h3 className="font-bold text-stone-800 mb-3 flex items-center gap-2 px-1"><Info size={16}/> 生存记录</h3>{systems.length === 0 ? <div className="text-center text-stone-300 text-xs italic">暂无记录</div> : <div className="bg-white border border-stone-100 rounded-lg overflow-hidden">{systems.map((msg,i)=><div key={msg.id} className={`p-3 border-b border-stone-50 last:border-0 ${i%2===0?'bg-white':'bg-stone-50/50'}`}><div className="flex justify-between mb-1"><span className="font-bold text-stone-700 text-xs">{msg.title}</span><span className="text-[10px] text-stone-400">{msg.time}</span></div><div className="text-xs text-stone-500">{msg.content}</div></div>)}</div>}</div></div>);
});
    ...
    <div className="p-4 h-full overflow-y-auto">
       <div className="mb-8">
           <div className="flex justify-between items-center mb-3 px-1">
              <h3 className="font-bold text-stone-800 flex items-center gap-2"><Scroll size={16}/> 探索目标</h3>
                 ...
                 <div key={quest.id} className="bg-white border border-stone-200 p-3 rounded-lg shadow-sm relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-center border-t border-stone-50 pt-2">
                       <div className="text-[10px] text-stone-500 font-mono flex items-center gap-2">
                         <span>获{quest.rewards.gold} / 验{quest.rewards.exp}</span>
                         <span className="text-emerald-600 flex items-center gap-0.5"><Battery size={8}/> -{quest.staminaCost}</span>
                       </div>
                       <button onClick={() => acceptQuest(quest.id)} disabled={!!hero.queuedQuest} className={`px-3 py-1 rounded text-[10px] font-bold transition-colors ${hero.queuedQuest ? 'bg-stone-100 text-stone-300' : 'bg-stone-800 text-white hover:bg-stone-700 active:scale-95'}`}>{hero.queuedQuest ? '队列已满' : '行动'}</button>
                    </div>
                 </div>
              ))}
              {hero.questBoard.length === 0 && <div className="text-center text-[10px] text-stone-300 py-4 italic">暂无目标...</div>}
           </div>
       </div>

       <div className="flex justify-between items-center mb-6 px-1">
          <div><h3 className="font-bold text-stone-800 flex items-center gap-2 text-xl"><Beer size={20}/> 营地</h3><div className="text-[10px] text-stone-400">幸存者聚集地</div></div>
       </div>
             ...
             <div key={visitor.id} className={`bg-white border p-4 rounded-xl shadow-sm relative overflow-hidden group transition-all hover:shadow-md ${getQualityBadgeClass(visitor.quality).replace('bg-', 'border-').replace('text-', 'hover:border-')}`}>
                ...
                <div className="flex gap-4 mt-2">
                   ...
                   <div className="flex-1">
                      ...
                      <div className="flex justify-between items-center"><div className="text-[10px] text-stone-400">加成: <span className="text-stone-700 font-bold">{visitor.buff.type.toUpperCase()} +{visitor.buff.val}</span></div><button onClick={() => hireCompanion(visitor.id)} disabled={!!hero.companion} className={`px-4 py-1.5 rounded text-xs font-bold active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed transition-colors ${hero.companion ? 'bg-stone-200 text-stone-500' : 'bg-stone-800 text-white hover:bg-stone-700'}`}>{hero.companion ? '已有同伴' : `招募 (${visitor.price}资)`}</button></div>
                   </div>
                </div>
             </div>
          ))}
       </div>
       <div className="text-center text-[10px] text-stone-300 mt-8 mb-4">每1小时有新流浪者经过。</div>
    </div>
  );
});
    ...
    return (
       <div className="flex h-[100dvh] flex-col items-center justify-center bg-[#fcf9f2] text-stone-800 p-6 relative overflow-hidden">
        <div className="z-10 flex flex-col items-center w-full max-w-xs">
          <div className="w-20 h-20 border-4 border-stone-800 rounded-full flex items-center justify-center mb-6 shadow-lg bg-white"><span className="font-serif text-4xl font-bold">生</span></div>
          <h1 className="text-2xl font-serif font-bold mb-8 tracking-[0.2em] text-stone-900 text-center">生存：进化<br/><span className="text-xs font-normal text-stone-500 tracking-normal">文明已死，唯适者生存</span></h1>
          <div className="flex flex-col gap-4 w-full">
            <input type="text" placeholder="幸存者代号" className="w-full bg-white/50 border-b-2 border-stone-300 p-3 text-center text-lg outline-none focus:border-stone-800 transition-colors font-serif placeholder:text-stone-400 rounded-t" value={inputName} onChange={e => {setInputName(e.target.value); clearError();}} />
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#fcf9f2] text-stone-800 font-serif max-w-md mx-auto shadow-2xl relative">
      ...
      <nav className="h-16 bg-white border-t border-stone-200 flex justify-around items-center px-1 flex-none z-20 shadow-[0_-5px_15px_rgba(0,0,0,0.02)]">
         <button onClick={() => setActiveTab('logs')} className={`flex flex-col items-center gap-1 p-2 min-w-[3.5rem] ${activeTab === 'logs' ? 'text-stone-800' : 'text-stone-400'}`}><ScrollText size={18} strokeWidth={activeTab === 'logs' ? 2.5 : 2} /><span className="text-[9px] font-bold">日记</span></button>
         <button onClick={() => setActiveTab('hero')} className={`flex flex-col items-center gap-1 p-2 min-w-[3.5rem] ${activeTab === 'hero' ? 'text-stone-800' : 'text-stone-400'}`}><User size={18} strokeWidth={activeTab === 'hero' ? 2.5 : 2} /><span className="text-[9px] font-bold">状态</span></button>
         <button onClick={() => setActiveTab('tavern')} className={`flex flex-col items-center gap-1 p-2 min-w-[3.5rem] ${activeTab === 'tavern' ? 'text-stone-800' : 'text-stone-400'}`}><MapPin size={18} strokeWidth={activeTab === 'tavern' ? 2.5 : 2} /><span className="text-[9px] font-bold">探索</span></button>
         <button onClick={() => setActiveTab('bag')} className={`flex flex-col items-center gap-1 p-2 min-w-[3.5rem] ${activeTab === 'bag' ? 'text-stone-800' : 'text-stone-400'}`}><Package size={18} strokeWidth={activeTab === 'bag' ? 2.5 : 2} /><span className="text-[9px] font-bold">背包</span></button>
         <button onClick={() => setActiveTab('messages')} className={`flex flex-col items-center gap-1 p-2 min-w-[3.5rem] ${activeTab === 'messages' ? 'text-stone-800' : 'text-stone-400'}`}><Bell size={18} strokeWidth={activeTab === 'messages' ? 2.5 : 2} /><span className="text-[9px] font-bold">信号</span></button>
      </nav>
    </div>
  );
}