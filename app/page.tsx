    ...
    <div className="flex flex-col h-[100dvh] bg-[#fcf9f2] text-stone-800 font-serif max-w-md mx-auto shadow-2xl relative">
        ...
        {hero.logs.map((log) => (
          <div key={log.id} className="animate-in fade-in slide-in-from-bottom-2 duration-700">
            ...
            <div className={`text-[15px] leading-7 text-justify ${
              ...
              log.type === 'ai' ? 'text-stone-900 italic font-medium' : // AI 内容加粗斜体
              'text-stone-600'
            }`}>
              {log.text}
            </div>
      ...
      <footer className="p-6 pb-8 flex-none bg-gradient-to-t from-[#fcf9f2] via-[#fcf9f2] to-transparent">
        <div className="flex justify-between gap-6">
          <button onClick={() => godAction('punish')} className="flex-1 h-14 border-2 border-stone-200 rounded-lg flex items-center justify-center gap-2 text-stone-600 hover:border-red-800 hover:text-red-800 hover:bg-red-50 transition-all active:scale-95">
            <Zap size={18} /> <span>天罚</span>
          </button>
          <button onClick={() => godAction('bless')} className="flex-1 h-14 border-2 border-stone-200 rounded-lg flex items-center justify-center gap-2 text-stone-600 hover:border-emerald-800 hover:text-emerald-800 hover:bg-emerald-50 transition-all active:scale-95">
            <Cloud size={18} /> <span>赐福</span>
          </button>
        </div>
      </footer>
    </div>
